...
def mkv_to_mp4(mkv_path, remove_mkv=False):
    ...
    print ("debug mj :", os.path.isfile(mkv_path))  #mj
    print ("mkv_path:",mkv_path)  #mj
...
def visualize(epoch_id, machine_steering, out_dir, perform_smoothing=False,
    ...
    print("vid_size=",vid_size) #mj
    out_path = join_dir(out_dir, 'epoch{:0>2}_human_machine.mkv'.format(epoch_id)) #mj
#    vw = cv2.VideoWriter(out_path, cv2.VideoWriter_fourcc(*'X264' ), 30, vid_size)
#    vw = cv2.VideoWriter(out_path, cv2.VideoWriter_fourcc('I','4','2','0'), 30, vid_size)
    vw = cv2.VideoWriter(out_path, cv2.VideoWriter_fourcc('M','J','P','G'), 30, vid_size) #mj
#    vw = cv2.VideoWriter(out_path, cv2.cv.FOURCC(*'X264' ), 30, vid_size)
    ...
    for f_cur in range(len(machine_steering)):
        ...
        axis.plot(xx, hh)
        axis.plot(xx, mm)
#        print ("!!!!!!!!------------------------------human_steering =", hh)  #mj
#        print ("!!!!!!!!------------------------------machine_steering =", mm)  #mj
        ...
        if (frame_count_limit is not None) and (frame_count_limit == 1): # mj
            print("debug here! frame_count_limit is :",frame_count_limit) #mj  
            cv2.imwrite(out_path.replace('mkv', 'jpg'), fimg)
            sys.exit()

        vw.write(fimg)
        
#        print("fimg is:", fimg)  #mj
        
    front_cap.release()
    if dash_exists:
        dash_cap.release()
    vw.release()
    print("up to here! out-path is :",out_path) #mj  
...
verbose=True, frame_count_limit=150) #mj