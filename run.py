...
import utils
## Parameters
## you can modify the content of params.py
import params
...
for epoch_id in epoch_ids:
    ...
    for frame_id in range(frame_count):
        ...
        img = img[None,:,:,:]
        deg = float(model.predict(img, batch_size=128))#mj        
#        deg = float('%.1f'%deg)        
...
#    print ("!!!------------------------------machine steering is :",machine_steering) #mj
    utils.visualize(epoch_id, machine_steering, params.out_dir,
                        verbose=True, frame_count_limit=None)  # frame_count_limit=None
